...
// //创新实验 ：
// // 在 correctness_guess.cpp 或其他任意一个源文件中定义：


// int get_mpi_rank() {
//     static int rank = -1;
//     if (rank == -1) {
//         const char* env = std::getenv("SIMU_RANK");
//         rank = (env ? std::atoi(env) : 0);  // 默认为0
//     }
//     return rank;
// }
...
//     const int MAX_LEN = 64;
//     const int rank = get_mpi_rank();  // 请自行实现 get_mpi_rank()：内部用 MPI_Comm_rank
//     cudaSetDevice(rank);              // 每个 MPI 进程绑定自己编号的 GPU
...
void PriorityQueue::Generate(PT pt)
{
    ...
    for (int i = 0; i < count; ++i) {
        strncpy(&flat_input[i * MAX_LEN], values[i].c_str(), MAX_LEN - 1);
    }
